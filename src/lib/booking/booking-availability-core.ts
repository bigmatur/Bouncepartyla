...
function getInventoryBaselineQuantity(inventoryItem: any) {
  const quantityOnHand = getNumberValue(
    inventoryItem?.quantity_on_hand,
    0,
  );

  const quantityAvailable = getNumberValue(
...
const AVAILABLE_UNIT_STATUSES = new Set([
  "available",
  "returned",
]);
...
function parseLocalDateTime(
  ...
  return new Date(
    `${eventDate}T${safeTime}:00-07:00`,
  );
}

function parseBookingActor(
  value: string,
): BookingActor {
...
function timeToMinutes(value: string) {
  const [hoursRaw, minutesRaw] = String(
    value || "00:00",
  ).split(":");
...
async function validateTimePolicy(
  ...
  if (exception) {
    ...
    openTime = exception.open_time
      ? String(exception.open_time).slice(
          0,
          5,
        )
      : openTime;
    ...
    closeTime = exception.close_time
      ? String(exception.close_time).slice(
          0,
          5,
        )
      : closeTime;
    ...
    if (workingHourError) {
      throw new Error(
        workingHourError.message,
      );
    }

    if (workingHour) {
      isOpen =
        workingHour.is_open !== false;
...
function getProductTiming(product: any) {
  return {
    rentalDurationMin: getNumberValue(
      product.rental_duration_min,
      720,
    ),

    setupDurationMin: getNumberValue(
    ...
    bufferBeforeMin: getNumberValue(
      product.buffer_before_min,
      0,
    ),

    bufferAfterMin: getNumberValue(
      product.buffer_after_min,
      0,
    ),
  };
}
...
function calculateAvailabilityWindow({
  ...
  const reservedFrom = addMinutes(
    start,
    -timing.setupDurationMin -
      timing.bufferBeforeMin,
  );

  const reservedUntil = addMinutes(
    end,
    timing.teardownDurationMin +
      timing.bufferAfterMin,
  );
  ...
  return {
    ...
    reservedFrom:
      toIsoNoMs(reservedFrom),

    reservedUntil:
      toIsoNoMs(reservedUntil),
...
async function checkQuantityInventory(
  ...
  const reservations =
    await getOverlappingReservations(
      supabase,
      {
        inventoryItemId:
          inventoryItem.id,
...
export async function checkBookingItemAvailabilityCore(
  supabase: any,
  formData: FormData,
) {
  ...
  if (!eventDate) {
    return {
      ...
      message:
        "Choose event date before checking inventory.",
  ...
  if (
    ...
    return {
      ...
      message:
        "Choose start and end time before checking inventory.",
  ...
  const timePolicyMessage =
    await validateTimePolicy(
      supabase,
      {
        bookingActor,
        eventDate,
  ...
  if (
    product.inventory_item_id &&
    !mainItemAlreadyIncluded
  ) {
    const {
      data: mainInventoryItem,
      error: mainInventoryError,
    } = await supabase
    ...
    if (!mainInventoryItem) {
      return {
        ...
        message:
          `Main inventory item is missing for "${product.name}".`,
        ...
        missingComponents: [
          {
            ...
            reason:
              "Main inventory item was not found.",
          },
        ],
    ...
    if (
      ...
      return {
        ...
        message:
          `Main inventory item is inactive for "${product.name}".`,
        ...
        missingComponents: [
          {
            ...
            inventoryItemId:
              mainInventoryItem.id,

            inventoryItemName:
              mainInventoryItem.name,

            trackingType:
              String(
                mainInventoryItem.tracking_type ||
                  "unknown",
              ),
            ...
            reason:
              "Main inventory item is inactive.",
          },
        ],
...
export async function getAdminInventorySnapshotCore(
  supabase: any,
  formData: FormData,
) {
  const eventDate = getString(
  ...
  try {
    ...
    return {
      ...
      message:
        "Could not parse product list for stock snapshot.",

...
function smsUrl(phone: string | null | undefined) {
  const cleanPhone = String(phone || "").replace(/[^\d+]/g, "");
  return cleanPhone ? `sms:${cleanPhone}` : "";
}
...
function routeTimeLabel(stop: DriverStop) {
  ...
  const hasStart = Boolean(timeValue(stop.scheduled_start_time));
  const hasEnd = Boolean(timeValue(stop.scheduled_end_time));
...
export default function DriverRouteApp({
  ...
  const routeStops = useMemo(() => {
    const sortOrderValue = (stop: DriverStop) => {
      const value = Number(stop.sort_order);
    ...
    return [...stops].sort((leftStop, rightStop) => {
      const leftOrder = sortOrderValue(leftStop);
      const rightOrder = sortOrderValue(rightStop);
  ...
  const [mapSoundMuted, setMapSoundMuted] = useState(false);
  const [carPlayMode, setCarPlayMode] = useState(false);

  useEffect(() => {
    try {
      setMapSoundMuted(window.localStorage.getItem("driverMapSoundMuted") === "1");
      setCarPlayMode(window.localStorage.getItem("driverCarPlayMode") === "1");
    } catch {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("driverMapSoundMuted", mapSoundMuted ? "1" : "0");
    } catch {
    ...
    if (typeof document !== "undefined") {
      document.querySelectorAll("audio, video").forEach((media) => {
        (media as HTMLAudioElement | HTMLVideoElement).muted = mapSoundMuted;
      });
    }

    if (mapSoundMuted && typeof window !== "undefined" && "speechSynthesis" in window) {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("driverCarPlayMode", carPlayMode ? "1" : "0");
    } catch {
      // Ignore localStorage write errors.
    }
  }, [carPlayMode]);
        ...
        <div className="mx-auto max-w-3xl">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="pointer-events-auto flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => setMapSoundMuted((value) => !value)}
                className={[
                  ...
                  carPlayMode ? "px-4 py-3 text-sm" : "",
                  isNavigatorMode ? "bg-[#23313f]/85" : "bg-white/15",
                ].join(" ")}
                aria-label={mapSoundMuted ? "Unmute map" : "Mute map"}
              >
                {mapSoundMuted ? "🔇" : "🔊"}
              </button>
              ...
              <button
                type="button"
                onClick={() => setCarPlayMode((value) => !value)}
                className={[
                  "rounded-full px-3 py-2 text-xs font-semibold text-white shadow-[0_10px_25px_rgba(0,0,0,0.22)] ring-1 ring-white/15 backdrop-blur transition",
                  carPlayMode
                    ? "bg-[#c9964f]"
                              ...
                              <>
                                ...
                                {customerPhone(activeStop) && (
                                  <div className="mt-3 flex items-center gap-2">
                                    ...
                                    <a
                                      href={smsUrl(customerPhone(activeStop))}

...
function isStarted(stop: DriverStop) {
  return ["on_the_way", "arrived"].includes(
    String(stop.status || ""),
  );
}

function breakMinutesFromStop(
  stop: DriverStop | null | undefined,
) {
  if (!stop) return null;
...
function routeTimeLabel(stop: DriverStop) {
  ...
  const routeDate = stop.stop_date
    ? formatShortDate(stop.stop_date)
    : "No date";
  ...
  const end = stop.scheduled_end_time
...
function driverLocationAddress(
...
function effectiveBalanceDue(stop: DriverStop) {
  const stopBalance = Number(
    stop.balance_due || 0,
  );

  if (
    String(stop.stop_type || "").toLowerCase() !==
    "delivery"
  ) {
    return Number.isFinite(stopBalance)
      ? Math.max(stopBalance, 0)
      : 0;
  }
...
export default function DriverRouteApp({
  ...
  const firstNavigableIndex =
    routeStops.findIndex((stop) =>
      isNavigableStop(stop),
    );
  ...
  const nextStop =
    nextNavigableIndex >= 0
      ? routeStops[nextNavigableIndex]
      : null;

  const isActiveBreak = Boolean(
    activeStop && isBreakStop(activeStop),
  );

  const isActiveNavigable = Boolean(
    activeStop &&
      isNavigableStop(activeStop),
  );

  const isNavigatorMode =
    isActiveNavigable &&
    (activeStop?.status === "on_the_way" ||
      navigationActiveStopId ===
        activeStop?.id);
  ...
  /*
   * During navigation we continuously track
   * the driver's REAL GPS position.
   *
   * This is intentionally one watchPosition
   * subscription only.
   */
  useEffect(() => {
    if (!isNavigatorMode) return;
  ...
  const driverOrigin =
    driverLocationAddress(driverLocation);

  const fallbackOrigin =
    previousNavigableStop
  ...
  /*
   * Preview:
   * GPS is preferred, but warehouse /
   * previous route stop may be used while
   * the driver has not started navigation.
   *
   * Navigation:
   * only the real driver's GPS is used.
   */
  const previewMapOrigin =
    driverOrigin || fallbackOrigin;

  const navigatorMapOrigin = driverOrigin;

  const mapOrigin = isNavigatorMode
    ? navigatorMapOrigin
    : previewMapOrigin;

  const mapDestination = activeStop
    ? mapAddress(activeStop)
    : "";
  ...
  const openCount =
    navigableStops.length - completedCount;

  const collectTotal =
    navigableStops.reduce((sum, stop) => {
      if (
  ...
  function updateStopStatus(
    ...
    startTransition(() => {
      void updateDriverAppStopStatusAction(
        formData,
      );
    });
  ...
  function startNavigation() {
     ...
     * We don't wait for the server action to
     * finish before switching the UI.
     */
    setNavigationActiveStopId(
      activeStop.id,
    );
  ...
  function handleMainAction() {
    ...
    if (
      ...
      updateStopStatus(
        activeStop,
        nextCompletionStatus(activeStop),
      );
    ...
    if (
      ...
      /*
       * Route completion and the end of paid
       * work are intentionally separate.
       */
      setNavigationActiveStopId(null);
      setSheetOpen(false);
    }
  ...
  const activeSequence = activeStop
    ? stopSequenceById.get(activeStop.id)
    : null;

  const openWorkBreak =
    workTimeEntry?.staff_time_breaks?.find(
      (item) => !item.ended_at,
    ) || null;

  const routeIsCompleted =
    navigableStops.length > 0 &&
    navigableStops.every((stop) =>
      isCompleted(stop),
    );
    ...
    <div className="relative h-[100dvh] min-h-screen overflow-hidden bg-[#111827]">
                      ...
                      <div className="rounded-[24px] bg-white p-5 ring-1 ring-[#eee5d9]">
                        ...
                        <div className="mt-1 text-sm leading-5 text-[#6c6258]">
                          {!workTimeEntry
                            ...
                            : openWorkBreak
                              ? `Break started ${formatTime(
                                  openWorkBreak.started_at,
                                )}.`
                              : `Work started ${formatTime(
                                  workTimeEntry.clock_in_at,
                                )}.`}
                        </div>
      ...
      <div
        className={[
          ...
          sheetOpen
            ? "translate-y-0"
            : "translate-y-[calc(100%-72px)]",
        ].join(" ")}
      >
        <div
          className="mx-auto max-h-[78dvh] max-w-3xl overflow-hidden rounded-t-[28px] bg-[#f5efe6] shadow-[0_-14px_45px_rgba(0,0,0,0.24)] sm:max-h-none sm:rounded-t-[34px]"
          style={{
            paddingBottom:
        ...
        >
          <button
            ...
            className="block min-h-[76px] w-full px-4 pt-2.5 sm:min-h-0 sm:px-5 sm:pt-3"
          >
            <span className="mx-auto block h-1.5 w-12 rounded-full bg-[#9f917f] sm:w-14 sm:bg-[#b8aa98]" />
            ...
            <span className="mt-3 flex items-center justify-between text-left">
              <span className="min-w-0">
                ...
                <span className="hidden text-sm text-[#6c6258] sm:block">
                  Swipe/tap to{" "}
                  {sheetOpen
                    ? "hide"
                    : "open"}{" "}
          ...
          <div className="max-h-[calc(78dvh-88px-env(safe-area-inset-bottom))] min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-3 sm:max-h-[52vh] sm:px-4">
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-0.5 line-clamp-2 text-[12px] font-semibold leading-4 text-[#9a723e] sm:text-sm sm:leading-5">
                            {routeTimeLabel(
                              stop,
                            )}
                          </div>
                                ...
                                {stop.stop_type === "delivery" &&
                                  ...
                                  !isBreakStop(stop) && (
                                    <a
                                      ...
                                      className="rounded-full bg-[#edf5fc] px-4 py-2 text-center text-xs font-semibold text-[#243342] ring-1 ring-[#cfe0ef]"
                                    >
                                      Open stop details
                                    </a>
                                  )}

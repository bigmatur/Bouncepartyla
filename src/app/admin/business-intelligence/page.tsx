          ...
          {marketingSnapshot.metaAds.current && marketingSnapshot.comparisons ? (
            <>
              ...
              <section className="min-w-0 rounded-[22px] border border-[#eadfd1] bg-white p-4 shadow-[0_8px_28px_rgba(45,36,25,.04)] sm:rounded-[26px] sm:p-5">
                ...
                <h2 className="mt-1 text-lg font-bold text-[#28231f]">
                  Leads requiring attribution review
                </h2>
                <p className="mt-1 text-xs leading-5 text-[#81766c]">
                  Concrete Instagram CRM leads behind attribution coverage gaps. These lists use the same first-touch attribution and revenue rules as the metrics above.
                </p>
                  ...
                  {[
                    {
                      key: "missing-ad-id",
                      title: "Missing ad ID",
                      note: "Instagram leads with no captured Meta first-touch ad ID.",
                      rows: marketingSnapshot.attribution.drillDown.missingAdId,
                    },
                    {
                      key: "unmatched-meta-ad",
                      title: "Ad not in Meta Insights",
                      note: "Captured ad IDs that Meta did not return for the selected period.",
                      rows: marketingSnapshot.attribution.drillDown.unmatchedMetaAd,
                    },
                    {
                      key: "no-revenue-booking",
                      title: "No revenue booking",
                      note: "Meta-matched leads without a linked booking currently in a revenue status.",
                      rows: marketingSnapshot.attribution.drillDown.noRevenueBooking,
                    },
                  ].map((group: any) => (
                    <div
                      key={group.key}
                      className="min-w-0 rounded-2xl border border-[#eee5dc] bg-[#fcfaf7] p-3"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-xs font-bold text-[#342e29]">
                            {group.title}
                          </div>
                          <div className="mt-1 text-[11px] leading-4 text-[#81766c]">
                            {group.note}
                          </div>
                        </div>

                        <div className="shrink-0 rounded-full bg-white px-2.5 py-1 text-[10px] font-bold text-[#6f655c] ring-1 ring-[#e7ddd3]">
                          {group.rows.length.toLocaleString("en-US")}
                        </div>
                      </div>

                      <div className="mt-3 space-y-2">
                        {group.rows.slice(0, 20).map((row: any) => (
                          <div
                            key={`${group.key}-${row.leadId}`}
                            className="rounded-xl border border-[#eee5dc] bg-white px-3 py-2.5"
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <Link
                                  href={`/admin/crm/events/${row.leadId}`}
                                  className="block truncate text-xs font-bold text-[#2d2a28] underline decoration-[#d9c8b4] underline-offset-2"
                                >
                                  {row.customerName}
                                </Link>

                                <div className="mt-0.5 truncate text-[10px] text-[#81766c]">
                                  {row.contactLabel}
                                </div>
                              </div>

                              <div className="shrink-0 text-right text-[10px] font-semibold capitalize text-[#85796d]">
                                {String(row.status || "unknown").replaceAll("_", " ")}
                              </div>
                            </div>

                            {row.adId ? (
                              <div className="mt-2 truncate text-[10px] text-[#6f655c]">
                                Ad ID: <span className="font-semibold">{row.adId}</span>
                              </div>
                            ) : null}

                            {row.adName ? (
                              <div className="mt-1 truncate text-[10px] text-[#6f655c]">
                                {row.adName}
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px]">
                              {row.bookingId ? (
                                <Link
                                  ...
                                  className="font-semibold text-[#6f655c] underline underline-offset-2"
                                >
                                  Open booking
                                </Link>
                              ) : (
                              ...
                              {row.bookingStatus ? (
                                <span className="rounded-full bg-[#f4efe9] px-2 py-0.5 font-semibold capitalize text-[#81766c]">
                                  {String(row.bookingStatus).replaceAll("_", " ")}
                                </span>
                              ) : null}
                            </div>
                          </div>
                        ...
                        {group.rows.length === 0 ? (
                          <div className="rounded-xl bg-white px-3 py-4 text-center text-[11px] font-medium text-[#81766c]">
                            No leads in this category.
                          </div>
                        ) : null}

                        {group.rows.length > 20 ? (
                          <div className="pt-1 text-center text-[10px] font-semibold text-[#81766c]">
                            Showing 20 of {group.rows.length.toLocaleString("en-US")}
                          </div>
                        ) : null}
                      </div>
                    </div>

...
export default async function AccountPage({
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-5 sm:px-5 sm:py-10">
        ...
        <section className="mt-5 sm:mt-8">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <div className="flex gap-3 text-xs text-black/45 sm:text-sm">
            ...
            <div className="mt-5 grid gap-4">
              {selectedCustomerBookings.map((booking) => {
                ...
                return (
                  <article
                    key={booking.id}
                    className="rounded-[20px] border border-black/10 bg-white p-4 shadow-[0_12px_35px_rgba(0,0,0,0.04)] sm:rounded-[26px] sm:p-6"
                  >
  ...
  if (pendingBookingIds.length > 0) {
    const sourceRows = await supabase
      .from("bookings")
      .select("id, booking_source, amount_paid, payment_status")
    ...
    if (!sourceRows.error) {
      ...
      bookings = bookings.map((booking) => {
        const state = stateById.get(booking.id);
        return {
          ...booking,
          booking_source: booking.booking_source || state?.booking_source || null,
          amount_paid: state ? state.amount_paid : booking.amount_paid,
          payment_status: state?.payment_status || booking.payment_status,
        };
      });
    }
  }
  ...
  return (
    ...
    <main className="mx-auto w-full max-w-6xl px-5 py-8 sm:py-10">
      <section className="rounded-[30px] border border-black/10 bg-white p-7 shadow-[0_20px_60px_rgba(0,0,0,0.05)] sm:p-9">
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-[-0.04em] sm:mt-3 sm:text-4xl">
          Welcome, {customerName}
        </h1>

        <div className="mt-3 hidden flex-wrap gap-x-7 gap-y-2 text-sm text-black/55 sm:flex">
          {profile.email ? (
      ...
      <section className="mt-8">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold tracking-[-0.03em] sm:mt-2 sm:text-2xl">
              My bookings
            </h2>
          </div>
                ...
                >
                  <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
                    <div>
                      ...
                      <p className="mt-3 text-base font-semibold sm:mt-4">
                        {formatDate(
                          booking.event_date,
                        )}
                      </p>
                    ...
                    <div className="min-w-0 rounded-2xl bg-[#f7f4ef] px-4 py-3 sm:min-w-48 sm:text-right">
                      <div className="flex items-center justify-between gap-3 sm:block">
                        <div>
                          <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-black/40 sm:text-xs">
                            Balance due
                          </p>
                        ...
                        <div className="text-right sm:mt-1">
                          <p className="text-[10px] font-medium uppercase tracking-[0.1em] text-black/35 sm:hidden">
                            Paid
                          </p>
                          <p className="text-sm font-semibold sm:hidden">
                            {formatMoney(Math.max(
                              Number(booking.amount_paid || 0),
                              Number(booking.total_amount || 0) - Number(booking.balance_due || 0),
                            ))}
                          </p>
                        </div>
                      </div>

                        <p className="mt-1 hidden text-xs text-black/45 sm:block">
                          Paid{" "}
                          {formatMoney(
                            Math.max(
                              Number(booking.amount_paid || 0),
                  ...
                  <div className="mt-4 border-t border-black/[0.07] pt-3 sm:mt-5 sm:flex sm:flex-wrap sm:items-center sm:justify-between sm:gap-4 sm:pt-4">
                    ...
                    <Link
                      ...
                      className="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-[#1d1d1b] px-4 text-sm font-semibold text-white transition hover:bg-black sm:min-h-10 sm:w-auto sm:border sm:border-black/10 sm:bg-white sm:text-[#1d1d1b] sm:hover:bg-black/[0.03]"
                    >
                      View booking
                    </Link>
                  </div>

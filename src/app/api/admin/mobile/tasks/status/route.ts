...
import { setAdminTaskCompletedCore } from "@/lib/tasks/admin-task";
...
async function authenticate(request: Request) {
  ...
  if (
    ...
    !access.can("dashboard.view")
  ) {
    return {
      response: forbidden("Access denied. Missing permission: dashboard.view"),
    } as const;
  }
...
export async function POST(request: Request) {
  ...
  try {
    const result = await setAdminTaskCompletedCore({

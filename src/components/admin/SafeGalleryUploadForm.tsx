...
type SafeGalleryUploadFormProps = {
...
export default function SafeGalleryUploadForm({
  action,
  hiddenFields,
  buttonLabel = "Upload photo",
  compact = false,
}: SafeGalleryUploadFormProps) {
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        const originalFiles = Array.from(fileInputRef.current?.files || []).filter((file) => file.size > 0);
        ...
        const safeFiles = originalFiles.map((originalFile) =>
          new File([originalFile], sanitizeFileName(originalFile.name), {

-- 058_working_time_historical_pay_rates.sql
--
...
-- Extends the California overtime report so payroll uses the pay rate that was
-- effective on each individual work_date. A rate change inside the selected
-- report period therefore no longer retroactively changes older shifts.
--
-- This migration replaces only the admin report function. It does not modify
-- Driver View, Route Board, route stops, bookings, existing time entries,
-- breaks, or pay-rate records.
...
/*
 * Keep rate-history intervals contiguous when an administrator saves a rate
 * with an Effective Date, including retroactive corrections.
 */
...
begin
  ...
  /* Serialize changes for this employee so concurrent edits cannot overlap. */
  perform 1
  from public.profiles p
  where p.id = p_profile_id
  for update;
  ...
  /* Close any earlier interval that reaches into the new effective date. */
  update public.staff_pay_rates r
     set effective_until = v_effective_from - 1,
         updated_at = now()
   where r.profile_id = p_profile_id
     and r.effective_from < v_effective_from
     and (r.effective_until is null or r.effective_until >= v_effective_from);

  /* Re-saving the same effective date updates that rate instead of adding a duplicate. */
  select r.id
    into v_rate_id
  from public.staff_pay_rates r
  where r.profile_id = p_profile_id
  ...
  if v_rate_id is not null then
    update public.staff_pay_rates
       set pay_type = 'hourly',
           ...
           overtime_eligible = coalesce(p_overtime_eligible, true),
...
revoke all on function public.admin_set_staff_pay_rate(uuid, numeric, boolean, date)
from public;

grant execute on function public.admin_set_staff_pay_rate(uuid, numeric, boolean, date)
to authenticated;

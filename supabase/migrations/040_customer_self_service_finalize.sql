...
begin
  ...
  if not v_contract_signed and coalesce(v_booking.contract_status, 'not_sent') <> 'signed' then
  ...
  if v_required > 0 then
    ...
    insert into public.payments (
      ...
      'Recorded from customer self-service checkout',
      v_now
    );
...
grant execute on function public.complete_customer_booking_checkout(uuid, numeric, text, text)
  to authenticated;

-- 114_working_time_unlinked_driver_review.sql
--
-- Safety guard for Staff -> Working Time.
--
-- Forgotten open shifts older than 24 hours:
--   * stay visible in the admin report;
--   * are marked needs_review=true;
--   * do NOT keep accumulating Paid / OT / DT / estimated payroll;
--   * do NOT appear as Working Now.
--
-- Fresh open shifts (<=24h) preserve the existing live behavior.
-- No historical row is edited and no clock-out time is invented.
--
-- This migration replaces only get_working_time_admin_report().
...
begin
  ...
  entries as (
    select
      ...
      i.status,
      (
        i.clock_out_at is null
        and i.clock_in_at < now() - interval '24 hours'
      ) as is_stale_open,
      case
        when i.clock_out_at is null
          and i.clock_in_at < now() - interval '24 hours'
        then 0
        else coalesce(
        (
  ...
  /*
   * Apply the rate that was effective on each classified workday.
   * This is intentionally day-based because California OT classification is
   * already day/week based above, and rate history is effective-date based.
   */
  classified_with_rate as (
    select
      rc.profile_id,
      rc.work_date,
      rc.paid_minutes,
      rc.regular_minutes,
      rc.overtime_minutes,
      rc.doubletime_minutes,
      coalesce(day_rate.pay_type, 'hourly') as pay_type,
      day_rate.hourly_rate,
      coalesce(day_rate.overtime_eligible, true) as overtime_eligible,
      case
        when coalesce(day_rate.pay_type, 'hourly') = 'hourly'
          and day_rate.hourly_rate is not null
        then (
          (rc.regular_minutes / 60.0) * day_rate.hourly_rate
          + case
              when coalesce(day_rate.overtime_eligible, true)
                then (rc.overtime_minutes / 60.0) * day_rate.hourly_rate * 1.5
              else (rc.overtime_minutes / 60.0) * day_rate.hourly_rate
            end
          + case
              when coalesce(day_rate.overtime_eligible, true)
                then (rc.doubletime_minutes / 60.0) * day_rate.hourly_rate * 2.0
              else (rc.doubletime_minutes / 60.0) * day_rate.hourly_rate
            end
        )
        else 0
    ...
    left join lateral (
      select
        ...
        r.overtime_eligible
      from public.staff_pay_rates r
      where r.profile_id = rc.profile_id
        and r.effective_from <= rc.work_date
        and (r.effective_until is null or r.effective_until >= rc.work_date)
      order by
        r.effective_from desc,
        r.created_at desc,
        r.id desc
  ...
  employee_rows as (
    select
      ...
      count(c.id) filter (where c.is_stale_open)::integer as stale_open_shift_count,
      coalesce(
        bool_or(
          c.id is not null
          and c.clock_out_at is null
          and not c.is_stale_open
        ),
        false
      ) as working_now,
      coalesce(
  ...
  with_pay as (
    select
      ...
      coalesce(
        (
          select sum(cwr.estimated_pay)
          from classified_with_rate cwr
          where cwr.profile_id = er.profile_id
        ),
        0
      )::numeric as estimated_pay
    from employee_rows er
  ),

  unlinked_driver_stale_shifts as (
    select
    ...
    where e.profile_id is null
      ...
      and not exists (
        ...
        where coalesce(p.is_active, true) = true
          ...
          and (
            (e.auth_user_id is not null and p.auth_user_id = e.auth_user_id)
            or (rd.auth_user_id is not null and p.auth_user_id = rd.auth_user_id)
          )
      )
  )
  ...
  select jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'stale_open_shifts',
        coalesce(sum(stale_open_shift_count), 0) + (select count(*) from unlinked_driver_stale_shifts)
    ),
    ...
    'unlinked_driver_shifts',
      coalesce(
        (
          select jsonb_agg(
            jsonb_build_object(
              'id', uds.time_entry_id,
              'route_driver_id', uds.route_driver_id,
              'driver_name', uds.driver_name,
              'work_date', uds.work_date,
              'clock_in_at', uds.clock_in_at,
              'clock_out_at', uds.clock_out_at,
              'source', uds.source,
              'status', uds.status,
              'needs_review', true
            )
            order by uds.clock_in_at desc, uds.time_entry_id
          )
          from unlinked_driver_stale_shifts uds
        ),
        '[]'::jsonb
      ),
      ...
      coalesce(
        jsonb_agg(
          jsonb_build_object(
            ...
            'stale_open_shift_count', stale_open_shift_count,

-- 049_working_time_edit_and_audit.sql
--
-- Adds safe administrator corrections for Working Time.
-- Every manual correction requires a reason and is written to an audit table.
-- Route Board, Driver View, bookings, and route stop data are not modified.
...
create table if not exists public.staff_time_adjustments (
  ...
  constraint staff_time_adjustments_type_check
    check (adjustment_type in ('shift_created', 'shift_updated'))
);
...
begin
  ...
  if p_clock_in_local is null or p_clock_out_local is null then
    raise exception 'Both start and finish times are required for a missed shift.';
  end if;
...
create or replace function public.get_staff_time_adjustments(
  p_time_entry_id uuid
)
returns table (
  id uuid,
...
begin
  ...
  where a.time_entry_id = p_time_entry_id
...
revoke all on function public.get_staff_time_adjustments(uuid) from public;
...
grant execute on function public.get_staff_time_adjustments(uuid)
to authenticated;
